import { Box, Button, CircularProgress, Dialog, Grid, Icon, Typography, useTheme } from '@mui/material';
...
export default function Mensagem() {
  ...
  const textoBotao = (): string =>
    (typeof mensagemState.exibirBotao === 'string' && mensagemState.exibirBotao.length > 0) ?
      mensagemState.exibirBotao : (typeof mensagemState.exibirBotao === 'boolean' && mensagemState.exibirBotao) ? 'Fechar' : ''
        ...
        <Grid item xs={3} sx={{ padding: 2, border: 'none', borderWidth: '2px', borderColor: 'green', textAlign: 'center' }}>
          <Box>
            ...
            <Condicional condicao={mensagemState.tipo === MensagemTipo.Warning}>
              <Icon sx={{ fontSize: theme.mensagens.tamanhoIcone, color: theme.mensagens.corWarning }}>report_error</Icon>
            </Condicional>
            ...
            <Condicional condicao={mensagemState.tipo === MensagemTipo.Sucesso}>
              <Icon sx={{ fontSize: theme.mensagens.tamanhoIcone, color: theme.mensagens.corSucess }}>check_circle_outlined</Icon>
            </Condicional>
