...
const expCNPJ: RegExp = new RegExp(/^(\d{2}\.?\d{3}\.?\d{3}\/?\d{4}-?\d{2})$/)
const expCPF: RegExp = new RegExp(/^(\d{3}\.?\d{3}\.?\d{3}\/-?\d{2})$/)
...
export default class ClsValidaCampo {
    ...
    public eCPF(_eCPF: string): boolean {
        ...
        if (this.campoVazio(_eCPF) === true || !expCPF.test(_eCPF)) {
    ...
    public eCNPJ(_eCNPJ: string): boolean {
        if (this.campoVazio(_eCNPJ) === true || !expCNPJ.test(_eCNPJ)) {
            return false
        }
        else {
